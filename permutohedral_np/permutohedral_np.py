...
class Permutohedral():
    ...
    def init(self, features):
        ...
        # rank = np.zeros((self.N, self.d + 1), dtype=np.int32)  # [N, d + 1]
        diff = elevated - rem0  # [N, d + 1]
        diff_i = diff[..., np.newaxis]  # [N, d + 1, 1]
        diff_j = diff[..., np.newaxis, :]  # [N, 1, d + 1]
        rank = ((diff_i < diff_j) * self.diff_valid[np.newaxis, ...]).sum(axis=2).astype(np.int32)  # (N, d + 1)
        rank += ((diff_i >= diff_j) * self.diff_valid[np.newaxis, ...]).sum(axis=1)  # (N, d + 1)
        ...
        vs = ((elevated - rem0) * down_factor).reshape(-1)
        idx = ((self.d - rank) + np.arange(self.N)[..., np.newaxis] * (self.d + 2)).reshape(-1)
        idx1 = ((self.d - rank + 1) + np.arange(self.N)[..., np.newaxis] * (self.d + 2)).reshape(-1)
        barycentrics[idx] += vs
        barycentrics[idx1] -= vs
        barycentrics = barycentrics.reshape((self.N, self.d + 2))
        barycentrics[..., 0] += (1. + barycentrics[..., self.d + 1])

        # Compute all vertices and their offset
        canonicalT = self.canonical.T
        canonical_ext = canonicalT[rank]
        canonical_ext = np.transpose(canonical_ext, axes=(0, 2, 1))

        keys = np.int32(rem0[..., np.newaxis, :self.d] + canonical_ext[..., :self.d])

        # Keys in string format.
        keys = keys.reshape((-1, self.d))
        hkeys, offsets = np.unique(keys, return_inverse=True, axis=0)
        ...
        n1s = np.repeat(hkeys[:, np.newaxis, :], repeats=self.d + 1, axis=1) - 1
        n2s = np.repeat(hkeys[:, np.newaxis, :], repeats=self.d + 1, axis=1) + 1
        n1s += (self.d_mat[np.newaxis, ..., :self.d] + self.diagone[np.newaxis, ..., :self.d])
        n2s -= (self.d_mat[np.newaxis, ..., :self.d] + self.diagone[np.newaxis, ..., :self.d])
        n1s = n1s.reshape((-1, self.d))
        n2s = n2s.reshape((-1, self.d))

        self.blur_neighbors = np.zeros((self.M * (self.d + 1), 2), dtype=np.int32)
        ...
        self.os = offsets.reshape(-1) + 1
        self.ws = barycentrics[..., self.d + 1].reshape(-1)
        self.blur_neighbors = self.blur_neighbors.reshape((self.M, self.d + 1, 2)) + 1
    ...
    def compute(self, inp, reverse=False):
        ...
        # ->> Splat
        for v in value_size:
            ch = np.repeat(inp[..., v:v + 1], repeats=self.d + 1, axis=-1)
            ch = ch.reshape(-1)
        ...
        # ->> Slice
        out = self.ws[..., np.newaxis] * values[self.os] * self.alpha
        out = out.reshape((self.N, self.d + 1, value_size))
        out = out.sum(axis=1)

...
void init(const float *features, const int N, const int d, int *os, float *ws, int *blur_neighbors, int *M)
{
    ...
    *M = hash_table.size();
    ...
    for (int i = 0; i < *M; i++)
    {
        for (int j = 0; j < d + 1; j++)
        {
            ...
            blur_neighbors[i * (d + 1) * 2 + j * 2 + 0] = hash_table.find(n1) + 1;
            blur_neighbors[i * (d + 1) * 2 + j * 2 + 1] = hash_table.find(n2) + 1;
        }
    }
...
void compute(const float *inp, const int N, const int value_size, const int M, const int d, const int *os, const float *ws, const bool reverse, float *out)
{
    float *values = new float[(M + 2) * value_size];
    memset(values, 0, sizeof(values) * ((M + 2) * value_size));

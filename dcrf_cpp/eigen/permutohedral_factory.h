...
protected:
          ...
          Permutohedral(int N, int d)
    {
        ...
        for (int i = 0; i < d_ + 1; i++)
        {
            ...
            {
                canonical_[i * (d_ + 1) + j] = i;
            }
            for (int j = d_ - i + 1; j < d_ + 1; j++)
            {
                canonical_[i * (d_ + 1) + j] = i - (d_ + 1);
            }
        }
        ...
        for (int i = 0; i < d_; i++)
        {
            scale_factor_[i] = 1. / sqrt(double((i + 2) * (i + 1))) * inv_std_dev;
        }
    }
...
//        delete[] canonical;
        ...
        M_ = hash_table.size();
        blur_neighbors1T_.resize(d + 1, M_);
        blur_neighbors2T_.resize(d + 1, M_);

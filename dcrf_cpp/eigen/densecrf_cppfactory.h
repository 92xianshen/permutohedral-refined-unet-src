...
//        out.array().colwise() /= (sm.array() + 1e-20);
    ...
    )
    {
        ...
        for (int i = 0; i < n_iterations_; i++)
        {
            ...
            MatrixXf tmp1 = -unary; // [n_classes, N]

            // Bilateral message passing and symmetric normalization
            MatrixXf bilateral_in(n_classes_, N_), bilateral_out(n_classes_, N_);
            bilateral_in = Q.array().rowwise() * bilateral_norm_vals(0, all).array();
            bilateral_filter_->compute(bilateral_in, false, bilateral_out); // [n_classes, N]
            ...
            MatrixXf spatial_in(n_classes_, N_), spatial_out(n_classes_, N_);
            spatial_in = Q.array().rowwise() * spatial_norm_vals(0, all).array();
            spatial_filter_->compute(spatial_in, false, spatial_out); // [n_classes, N]
            spatial_out.array().rowwise() *= spatial_norm_vals(0, all).array();

            // Message passing
            MatrixXf message_passing = bilateral_compat_ * bilateral_out + spatial_compat_ * spatial_out; // [n_classes, N]

            // Compatibility transformation
            MatrixXf pairwise = compatibility_matrix_ * message_passing; // [n_classes, N]
